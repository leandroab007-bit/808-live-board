...
export function AdminScreen() {
  ...
  const adjustPrice = (id: string, delta: number) => {
    setDrinks((prev) =>
      prev.map((d) =>
        d.id === id ? { ...d, price: Math.max(1, Number((d.price + delta).toFixed(2))) } : d,
      ),
    );
  };
  ...
  const triggerCrash = (id: string) => {
    setDrinks((prev) =>
      prev.map((d) =>
        d.id === id
          ? { ...d, price: Number((d.price * 0.5).toFixed(2)), crashUntil: Date.now() + 120_000 }
          : d,
      ),
    );
  };
  ...
  return (
    <main className="relative h-full w-full overflow-hidden bg-background text-foreground">
      ...
      <div className="relative z-10 flex h-full flex-col p-4 gap-3">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 flex-1 min-h-0 overflow-auto">
          {drinks.map((d) => {
            ...
            return (
              <div key={d.id} className="panel-card rounded-lg p-5 flex flex-col gap-4">
                <div className="flex items-center justify-between border-b border-panel-border pb-2">
                  <span className="font-display font-bold tracking-wider text-lg text-neon-cyan">{d.name}</span>
                  {isCrashed && (
                ...
                <div className="flex flex-col gap-1">
                  <span className="font-body text-xs tracking-widest text-muted-foreground">PREÇO ATUAL</span>
                  <span
                ...
                <div className="flex flex-col gap-1 mt-auto">
                  <div className="flex items-center justify-between">
                    <span className="font-body text-xs tracking-widest text-muted-foreground">ESTOQUE</span>
